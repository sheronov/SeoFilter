...
class sfDictionaryUpdateProcessor extends modObjectUpdateProcessor
{
    ...
    public function beforeSet()
    {
        ...
        } elseif (!empty($alias) && $this->modx->getCount($this->classKey, array('alias' => $alias, 'field_id' => $field_id, 'id:!=' => $id))) {
            $this->modx->error->addField('alias', $this->modx->lexicon('seofilter_dictionary_err_ae'));
        } elseif (empty($alias) && $this->modx->getCount($this->classKey, array('value' => $name, 'field_id'=>$field_id, 'id:!=' => $id))) {
